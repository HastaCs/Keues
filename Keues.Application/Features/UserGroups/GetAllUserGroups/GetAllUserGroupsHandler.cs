  ...
  public GetAllUserGroupsHandler(IApplicationDbContext context)
    ...
    public async Task<GetAllUserGroupsResult> Handle(GetAllUserGroupsQuery request)
    {
        ...
        if (request.LocationId.HasValue)
            userGroups = userGroups.Where(ug => ug.LocationId == request.LocationId.Value);
        
        
        var result=userGroups.Select(ug => new UserGroupBaseResult(ug.Id,ug.Name, ug.Color,ug.LocationId,ug.CreatedAt)).ToList();
