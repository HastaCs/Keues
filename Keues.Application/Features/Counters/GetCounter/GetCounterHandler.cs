...
public class GetCounterHandler
{
  ...
  public async Task<CounterBaseResult> Handle(GetCounterCommand command)
  {
    var counter = await _context.Counters.Include(c => c.Queues)
      .Include(c => c.AuthorizedUsers)
      .Include(c => c.AuthorizedUserGroups).FirstOrDefaultAsync(c => c.Id == command.Id);
    if (counter == null)
    {
